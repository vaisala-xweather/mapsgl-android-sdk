...
/**
 * Demo 4: temperature clipped to OSM motorways that lie on **land** (built-in
 * [com.xweather.mapsgl.layers.spec.MaskLayerKind.LAND] marine stencil intersected with highway line mask).
 * Launched from [StencilMaskMenuActivity].
 */
class LandHighwayMaskTemperatureActivity : AppCompatActivity() {

    private lateinit var binding: ActivityLandHighwayMaskTemperatureBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityLandHighwayMaskTemperatureBinding.inflate(layoutInflater)
        setContentView(binding.root)
        drawBehindCutout(
            binding.root,
            InsetEdges(binding.landHighwayMaskTemperatureBackButton, top = true, start = true),
        )

        mapView = binding.landHighwayMaskTemperatureMapView
        ...
        mapView.viewTreeObserver.addOnGlobalLayoutListener(object : ViewTreeObserver.OnGlobalLayoutListener {
            override fun onGlobalLayout() {
                ...
                stencilDemos = StencilMaskDemos(this@LandHighwayMaskTemperatureActivity, controller)

                controller.setCenter(Coordinate(39.0, -98.0))
    ...
    private fun onMapLoaded() {
        ...
        stencilDemos.setupLandHighwayMaskTemperature()
    ...
    override fun onDestroy() {
        if (::stencilDemos.isInitialized) {
            stencilDemos.teardownLandHighwayMaskTemperature()
        }
