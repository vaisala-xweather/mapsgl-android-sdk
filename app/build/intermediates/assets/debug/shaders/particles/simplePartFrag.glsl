...
void main(void){
    ...
    if(x_scale<1.0) { //Partial code. May differ from JS version...
        ...
        outPart.x= (outPart.x+1.0)/2.0;
        outPart.y= (outPart.y+1.0)/2.0;
    }

    float colorScaleRes = 256.0;
    vec2 colorTemp = outPart.xy;
    //----------TESTING, almost works, causes black streaks? MAYBE NEED TO FIX OUTPOS OR BEFORE IN VERT!!
    //colorTemp.x= (colorTemp.x + 1.0)*.5;
    //colorTemp.y= (colorTemp.y + 1.0)*.5;
    //----------TESTING, almost works, causes black streaks?


    vec4 texColor = texture2D(u_Texture, colorTemp); // was vUV
