...
//#include <skip_nodata>
    ...
    float dd = sampleColorValue(tex, dataChannel[0]);
    // float edge  = isEdge(dataTexture, tuv, dataResolution, noData, dataChannel[0]);
...
// lookup the velocity from the data texture at this particle position
    ...
    vec4 texel1 = textureLookup(tuv);
    vec4 texel2 = textureLookup(tuv2);
...
//end #include <get_data_value>
    ...
    float dataValue = sampleDataRange.x + (sampleDataRange.y - sampleDataRange.x) * value;

    if (dataValue < drawRange.x) discard;
    if (dataValue > drawRange.y) discard;

    vec4 color = colorLookup(value, colorScaleRes, colorScale);
    float alpha = 1.;
...
#ifdef STYLE_ARROW
    float size = 0.3;
    pos = vUv - vec2(0.0, 0.5);
    float bodySize = mix(0.3, 2.0, size);
    float headSize = 0.1;
    float lineWidth = mix(0.18, 0.12, size);
    float height = 1.0;

    vec4 fill = vec4(1.0);
    float d = arrow_stealth(pos.xy, bodySize, headSize, height, lineWidth, 1.0);
    color = filled(d, 0.15, 0.01, color);
#endif
#ifdef STYLE_POINT
    float distance = length(2.0 * vUv - 1.0);
