...
void main() {   //TODO: MAIN START
    ...
    // constants from uniform, commented out at top
    //vec2 drawRange = vec2(0.0, 1.0); //normal
    //vec2 drawRange = vec2(0.0, .6); //test
    //vec2 drawRange = vec2(.0, .61); //test for windchill
    ...
    vec2 v_TexCoordP;//this bit handles partial
        ...
        if(x_scale<1.0) { //Partial code. May differ from JS version...
            float tex_mult=1.0;
            v_TexCoordP =  vec2((vUV.x*(tex_mult))*x_scale +(tex_mult)*x_offset,  (vUV.y*(tex_mult))*x_scale+(tex_mult)*y_offset);
        } else{
            v_TexCoordP = (vUV ); // .055 extra crop per pixel
        }
   ...
   // determine velocity from vectors
           ...
           float valueV = sampleColorValue(texel2, channel2);
