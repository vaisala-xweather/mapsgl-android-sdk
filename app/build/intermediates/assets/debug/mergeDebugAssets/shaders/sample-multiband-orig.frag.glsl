...
//uniform sampler2DArray colorScale;
varying vec2 vUV;
...
//=============================================================
    ...
    #endif
    //vec4 texel = mix(texel1, texel2, dataMeld); //this has no effect until second texture is defined
    vec4 texel = texel1;
    ...
    float value;
    value = sampleColorValue(texel, channel1);
    ...
    //These 3 were commented out to test red visibility
    ...
    //value = clamp(value, 0.01, .995);


    vec4 color = vec4(0.0, 0.0, 0.0, value);
    //float greenMask = posterizedGreen;
    float greenMask =  texture2D(dataTexture, tuv).g;


    if (greenMask > .5) { //.5
        color = colorLookup(value, colorScaleRes, colorScale1);  //red
    } else if (greenMask > .3) {
        color = colorLookup(value, colorScaleRes, colorScale2);  //blue
    } else {
        color = colorLookup(value, colorScaleRes, colorScale); //green
    }
    ...
    gl_FragColor = color;
    //gl_FragColor =  texture2D(dataTexture, tuv);
    //gl_FragColor.g=posterizedGreen;
    //gl_FragColor.r=0.0;
    //gl_FragColor.b=0.0;
}
