...
void main(void) {
        ...
        if (x_scale < 1.0) { //Partial code. May differ from JS version...
             ...
             outPart = outPos.xy;
             //outPart.x = outPos.x * tx_scale+tx_offset;
             //outPart.y = outPos.y * ty_scale+ty_offset;
             //outPart.x = (outPos.x ) *.5;
             //outPart.y = (outPos.y ) *.5;
        ...
        //float ndVal = 0.0;
        //float ndVal = noData;
        vec4 tex = texture2D(u_Texture, outPart.xy);
    ...
    //END #include <skip_nodata>
        ...
        #ifdef NO_DATA
        //float ndVal = 0.0;
        vec4 texel1 = interpolatedScaledNoDataTexel(interpolation, u_Texture,  outPart.xy, dataResolution, scale, noData);
